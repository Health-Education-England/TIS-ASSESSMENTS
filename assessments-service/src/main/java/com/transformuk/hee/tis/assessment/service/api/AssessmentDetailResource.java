...
public class AssessmentDetailResource {
  ...
  /**
   * GET  /:traineeId/assessments/:assessmentId/details : get the an assessment that's linked to a
   * trainee.
   *
   * @param traineeId    the id of the trainee
   * @param assessmentId the id of the assessmentDTO to retrieve
   * @return the ResponseEntity with status 200 (OK) and with body the assessmentDetailDTO, or with
   *     status 404 (Not Found)
   */
  @GetMapping("/{traineeId}/assessments/{assessmentId}/details")
  @PreAuthorize("hasAuthority('assessment:view:entities')")
  public ResponseEntity<AssessmentDetailDTO> getTraineeAssessmentDetails(
      @PathVariable Long traineeId, @PathVariable Long assessmentId) {
    log.debug("REST request to get AssessmentDetail");
    Optional<AssessmentDetailDTO> assessmentDetailDTO = assessmentDetailService
        .findAssessmentDetailBy(traineeId, assessmentId);
    return ResponseUtil.wrapOrNotFound(assessmentDetailDTO);
  }

  /**
   * POST  /:traineeId/assessments/:assessmentId : get the an assessment that's linked to a
   * trainee.
   *
   * @param traineeId    the id of the trainee
   * @param assessmentId the id of the assessmentDTO to retrieve
  ...
  public ResponseEntity<AssessmentDetailDTO> createTraineeAssessmentDetails(
    ...
    if (traineeAssessment.isPresent()) {
      savedAssessmentDetail = assessmentDetailService
          .create(traineeAssessment.get(), assessmentDetailDTO);
    }
  ...
  /**
   * PUT  /:traineeId/assessments/:assessmentId/details : get the an assessment that's linked to a
   * trainee.
   *
   * @param traineeId    the id of the trainee
   * @param assessmentId the id of the assessmentDTO to retrieve
  ...
  @PreAuthorize("hasAuthority('assessment:add:modify:entities')")
  public ResponseEntity<AssessmentDetailDTO> updateTraineeAssessmentDetails(
      @RequestBody @Validated(Update.class) AssessmentDetailDTO assessmentDetailDTO,
      @PathVariable Long traineeId, @PathVariable Long assessmentId) {
    ...
    if (traineeAssessment.isPresent()) {
      savedAssessmentDetail = assessmentDetailService
          .save(traineeAssessment.get(), assessmentDetailDTO);
    }
    return ResponseUtil.wrapOrNotFound(Optional.ofNullable(savedAssessmentDetail));
