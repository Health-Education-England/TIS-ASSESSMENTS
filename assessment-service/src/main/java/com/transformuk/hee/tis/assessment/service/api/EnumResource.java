...
public class EnumResource {
  ...
  @ApiOperation(value = "Lists all OutcomeReason types that can be associated with an Outcome",
      notes = "Used by clients to retrieve all OutcomeReason types that are currently available by this service. \n" +
          "This allows clients to dynamically list out all options for particular fields so that we do not \n" +
          "need to maintain a list on both the backend and client",
      response = ResponseEntity.class, responseContainer = "List",
      httpMethod = "GET", produces = "application/json", protocols = "http, https")
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "All Assessment Types", response = ResponseEntity.class)})
  @RequestMapping(value = "/outcome-reason", method = RequestMethod.GET)
  public ResponseEntity<OutcomeReason[]> getAllOutcomeReasons() {
    return new ResponseEntity<>(OutcomeReason.values(), HttpStatus.OK);
  }
